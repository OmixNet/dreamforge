...
// DREAMFORGE_SLIM: cli_agent_runtime 物理删除 (PR 4) — EnvName + env_value_from_process_or_user_shell inline 到 git/mod.rs
// DREAMFORGE_SLIM: EnvName 简化 (PR 7) — 删 `bool` trusted field (v0.1 不区分 trusted/untrusted, 全 read process env)
#[derive(Clone, Copy)]
struct EnvName<'a>(&'a str);

impl<'a> EnvName<'a> {
    const fn trusted(name: &'a str) -> Self {
        Self(name)
    }
...
/// v0.1 简化: 只读 process env。v0.1 不再 read user shell (Zsh/Bash) env 注入到 git。
fn env_value_from_process_or_user_shell(name: EnvName<'_>) -> Option<String> {
    std::env::var(name.0).ok()
}
