...
fn dreamforge_self_vault_path() -> Option<PathBuf> {
    dirs::home_dir()
        .map(|home| home.join("Desktop/APP/dreamforge-test-vault"))
        .filter(|path| path.is_dir())
}

fn default_vault_list_if_available() -> VaultList {
    let Some(path) = dreamforge_self_vault_path() else {
        return VaultList::default();
    };
    let path = path.to_string_lossy().to_string();
    VaultList {
        vaults: vec![VaultEntry {
            label: "dreamforge-test-vault".to_string(),
            path: path.clone(),
            ..Default::default()
        }],
        active_vault: Some(path.clone()),
        default_workspace_path: Some(path),
        hidden_defaults: vec![],
    }
}
...
pub fn load_vault_list() -> Result<VaultList, String> {
    ...
    if list.vaults.is_empty() && list.active_vault.is_none() {
        return Ok(expand_vault_list_paths(default_vault_list_if_available()));
    }
    Ok(expand_vault_list_paths(list))
}
...
mod tests {
    ...
    #[test]
    fn preferred_vault_list_path_uses_dreamforge_namespace() {
