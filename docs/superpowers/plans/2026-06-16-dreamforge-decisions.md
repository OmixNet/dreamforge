...
### §43 PR 43 close-out + PR 44-47 path (2026-06-22) — Active provider indicator ship + Settings AI product closure plan

PR 43 (d64cf4f) Settings AI Active provider indicator + Use this switcher: ship-ready. 22 files, +355 / -3, 5 new tests, 3890 → 3895.

GUI verify result (4/5 verified by real .app, 1/5 test-locked only):
1. 1 saved provider → row shows `Active` badge, no Use this button — **PASS** (real .app)
2. Add a 2nd provider → new row shows `Active`, old shows `Use this` button — **PASS** (real .app)
3. Click `Use this` on older provider → badge swaps, localStorage `dreamforge.llmApiKeyProviderId` + `dreamforge.llmApiKeyEnv` update — **PASS** (real .app)
4. local_file provider without Keychain key → `Use this` is disabled + `Add API key first` hint appears under row — **NOT GUI-verified** (本机 macOS TCC 限制: user 不愿在 production Keychain 删一条 item 制造 not-configured 状态), but `AiProviderSettings.test.tsx` test "PR 43: Use this is DISABLED for local_file providers when Keychain reports not configured" 锁了这条 invariant
5. Delete active provider → `Active` badge 消失, localStorage pointer 清干净 (no dangling) — **PASS** (real .app)

Why test 4 不 trivial GUI-verifiable (locked invariant for PR 44): Keychain status 只在 key entry **不存在** 时报 "not configured". 在 production 系统上制造这个状态需要 (a) 删真实 Keychain item (跨 PR 高风险, 跟 production 数据耦合) 或 (b) 调 Rust `has_ai_model_provider_api_key` 时 mock 返回 false. test 走 (b) — `invoke` mock 在 test setup 返 `configured: false` 直接模拟. 但 GUI 跑真 .app 时 Rust 端调真 Keychain Security framework, 不会返 mock value. **PR 44 目标: dev/test-only 入口, 让这个 state 可重复触发的, 不用碰真 Keychain 也不用让 product flow 多出 debug UI.** 候选方案 (PR 44 决定时拍): (i) `dreamforge.dev.forceKeychainMissing` localStorage flag, AiProviderSettings useEffect poll 完后如果 flag set 就 override local_file entries 到 false; (ii) Story/fixture harness component 包 AiProviderSettings 传 mock `keyConfiguredByProviderId` Map; (iii) `__settings_dev` query param + dev-only toggle. 选 (i) 概率最高, 因为 zero UI change, 跟现有 test mocking pattern 平行.

PR 44-47 顺序 + scope (user 拍板 2026-06-22):
- **PR 44** (testable Keychain-missing state): dev/test 入口让 local_file + keyConfigured=false 状态能自动化验; 不改 production flow, 不暴露给普通用户
- **PR 45** (Settings AI final product closure): "当前用于 Run Dream" badge 文案 + "设为当前" button + 顶部当前 config summary 一行 (e.g. "当前：OpenRouter · openai/gpt-4.1-mini"); 不做 HTTP test, 不做 runtime preview, 保持小 scope
- **PR 46** (DreamPanel error state polish): 6 类 error 各自的 actionable UI (missingKey / auth / modelNotFound / timeout / network / baseURL), 按钮跳 Settings 对应行; 保留 PR 41 collapsible raw body
- **PR 47** (empty editor productization): 最近 3 条 notes/raw/memory 快捷入口 + vault 健康状态摘要 (notes / raw / memory / candidate count), PR 42 的 3 button 保留
- **PR 37 / 38** (Anthropic / Gemini real E2E): 条件 — user 提供 key 才跑, tag v0.6.1 / v0.6.2; 没 key 不挡 PR 44-47 推进

为什么先做 PR 44 + PR 45 (user 原话): "PR 43 核心功能已经成了, 现在最弱的是 '状态可验证性' 和 '用户知道当前到底用哪个 provider'. 这两步做完, Settings AI 这块基本可以收口." PR 46/47 是 DreamPanel + 空态 polish, 可在 PR 44/45 完后继续.
