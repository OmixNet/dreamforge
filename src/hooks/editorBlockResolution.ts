...
// DREAMFORGE_SLIM: injectDurableEditorMarkdownBlocks + preProcessDurableEditorMarkdown 物理删除 (PR 8, editorDurableMarkdown 删)
...
function preProcessEditorMarkdown(
  ...
  // DREAMFORGE_SLIM: preProcessDurableEditorMarkdown 物理删除 (PR 8), 直接走 withImages
  const baseMarkdown = vaultPath ? resolveImageUrls(markdown, vaultPath, notePath) : markdown
  const withWikilinks = preProcessWikilinks(baseMarkdown)
  return preProcessMathMarkdown({ markdown: withWikilinks }).markdown
}
...
function injectEditorMarkdownBlocks(blocks: EditorBlocks): EditorBlocks {
  ...
  // DREAMFORGE_SLIM: injectDurableEditorMarkdownBlocks 物理删除 (PR 8)
  return withHighlights as EditorBlocks
}
