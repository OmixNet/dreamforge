...
interface UseStartupScreenStateArgs {
  ...
  // DREAMFORGE_SLIM: showMcpSetupDialog 物理删除 (PR 6, McpSetupDialog 已删)
...
function needsAiAgentsOnboarding(
  ...
  return onboardingState.status === 'ready' && aiAgentsPromptVisible
}
...
function shouldShowStartupScreenForState({
  ...
  const startupReasons = [
    ...
    shouldResumeFreshStartOnboarding,
    needsAiAgentsOnboarding(onboardingState, aiAgentsPromptVisible),
  ]
