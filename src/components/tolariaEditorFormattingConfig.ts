...
import {
  CodeBlock,
  File,
  // DREAMFORGE_SLIM: FlowArrow 物理删除 (PR 8, mermaid dep 删)
  ImageSquare,
  ListBullets,
  ListChecks,
  ListNumbers,
  ...
  Paragraph,
  Quotes,
  // DREAMFORGE_SLIM: ScribbleLoop 物理删除 (PR 8, tldraw dep 删)
  Smiley,
  SpeakerHigh,
  Table,
  TextHOne,
...
// DREAMFORGE_SLIM: MERMAID_SLASH_COMMAND_DIAGRAM 物理删除 (PR 8, mermaid 删, createMermaidSlashMenuItem caller 删)
// DREAMFORGE_SLIM: createBoardId 物理删除 (PR 8, createWhiteboardSlashMenuItem 删)
...
const TOLARIA_SLASH_MENU_ICONS: Partial<Record<string, PhosphorIcon>> = {
  ...
  image: ImageSquare,
  math: Pi,
  // DREAMFORGE_SLIM: mermaid: FlowArrow + whiteboard: ScribbleLoop 物理删除 (PR 8, mermaid + tldraw dep 删)
...
// DREAMFORGE_SLIM: createBoardId + createWhiteboardSlashMenuItem + createMermaidSlashMenuItem 物理删除 (PR 8, tldraw + mermaid dep 删)
...
export function getTolariaSlashMenuItems(
  ...
  const items = addItemsToMediaGroup(
    ...
    [
      // DREAMFORGE_SLIM: createMermaidSlashMenuItem + createWhiteboardSlashMenuItem 物理删除 (PR 8)
