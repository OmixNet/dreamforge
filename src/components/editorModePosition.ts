...
function serializeBlock(_editor: BlockNotePositionEditor, block: BlockLike): string {
  // DREAMFORGE_SLIM: serializeDurableEditorBlocks 物理删除 (PR 8), 直接 compactMarkdown 字符串化 block
  return compactMarkdown(JSON.stringify(restoreWikilinksInBlocks([block])))
}

function serializeEditorBody(_editor: BlockNotePositionEditor): string {
  // DREAMFORGE_SLIM: serializeDurableEditorBlocks 物理删除 (PR 8), 直接 compactMarkdown 字符串化 document
  return compactMarkdown(JSON.stringify(restoreWikilinksInBlocks(_editor.document)))
}
