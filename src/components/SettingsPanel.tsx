...
interface SettingsPanelProps {
  ...
  // v0.2 PR 11.5: Settings → Vault section uses these to add / remove vaults
  // without forcing a switch (the user is managing the list, not opening it).
...
export function SettingsPanel({
  ...
  return (
    <SettingsPanelInner
      ...
      {...{ onAddVault, selectedVaultPath, onRemoveVault, onReorderVaults, onSetDefaultWorkspace, onUpdateWorkspaceIdentity }}
...
interface SettingsBodyFromDraftProps {
  ...
  vaults: VaultOption[]
  defaultWorkspacePath?: string | null
  // v0.2 PR 11.5: Settings → Vault section uses these
  onAddVault?: (path: string, label: string) => void
  selectedVaultPath?: string | null
  onRemoveVault?: (path: string) => void; onReorderVaults?: (orderedPaths: string[]) => void; onSetDefaultWorkspace?: (path: string) => void; onUpdateWorkspaceIdentity?: (path: string, patch: Partial<VaultOption>) => void
  setThemeMode: (value: ThemeMode) => void
    ...
    <>
      ...
      <SettingsSection id={SETTINGS_SECTION_IDS.workflow}>
        <OrganizationWorkflowSection
          ...
          vaults={vaults}
          selectedVaultPath={selectedVaultPath ?? null}
      ...
      <SettingsGroup>
        ...
        <div className="space-y-3 p-4">
          <VaultSettingsSection
            vaults={vaults ?? []}
            activeVaultPath={selectedVaultPath ?? null}
            onAddVault={onAddVault ?? (() => {})}
            onRemoveVault={onRemoveVault ?? (() => {})}
          />
        </div>
