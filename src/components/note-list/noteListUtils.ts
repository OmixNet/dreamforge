...
// DREAMFORGE_SLIM: SLIM_FOLDERS 移过来, resolveFolderTitle 用 Slim folder label (Notes/Wiki/Memory/Raw/Archive) 不用 raw slug (notes/wiki/...)
import { SLIM_FOLDERS, isSlimFolderId } from '../../utils/slimSelection'
...
function resolveFolderTitle(selection: SidebarSelection): string | null {
  ...
  // DREAMFORGE_SLIM: Memory selection 用 path='' (root file) → 强制 'Memory' label
  if (selection.path === '') {
    return SLIM_FOLDERS.find((f) => f.id === 'memory')?.label ?? 'Memory'
  }
  // DREAMFORGE_SLIM: Slim folder slug → label (notes → Notes)
  if (isSlimFolderId(selection.path)) {
    return SLIM_FOLDERS.find((f) => f.id === selection.path)?.label ?? selection.path
  }
