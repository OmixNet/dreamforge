...
export function useFilterCounts(
  ...
  return useMemo(() => {
    ...
    // DREAMFORGE_SLIM: Slim folder selection 5 entry 都有固定 path mapping, 不依赖 selection.path 是否 empty
    if (selection.kind === 'folder' && isSlimFolderSelection(selection)) {
      const slimFolder = slimFolderIdFromSelection(selection)
      const folderEntries = entries.filter((e) => isSlimEntryInFolder(e, slimFolder, selection.rootPath))
      return countAllByFilter(folderEntries)
    }
    if (selection.kind === 'folder') {
      const folderPath = selection.path || selection.rootPath
      if (!folderPath) return countAllByFilter(entries)
      const folderEntries = entries.filter((e) => isEntryInFolder(e, folderPath))
      return countAllByFilter(folderEntries)
    }
...
// DREAMFORGE_SLIM: entry.path 在 folderPath 之下 → Slim "notes" 对应 vault 根下 "notes/" prefix
function isEntryInFolder(entry: VaultEntry, folderPath: string): boolean {
  if (!folderPath) return true
  if (entry.path === folderPath) return true
  return entry.path.startsWith(`${folderPath}/`)
}

// DREAMFORGE_SLIM: Slim mode 5 entry 都有固定 entry path mapping
// - notes/wiki/raw/archive: vault 根下 `${id}/` prefix
// - memory: vault 根下 `MEMORY.md` (root file, 不在子目录)
import { isSlimFolderId, type SlimFolderId } from '../../utils/slimSelection'
// DREAMFORGE_SLIM: entry.path 是 absolute (Rust: root.join(relative)), 用 pathRelativeToRoot + isInFolder 复用
// (跟 filterFolderEntries 行为一致)
import { pathRelativeToRoot, isInFolder } from '../../utils/noteListHelpers'

function isSlimFolderSelection(selection: SidebarSelection & { kind: 'folder' }): boolean {
  if (selection.path === '') return true  // memory selection (root file)
  return isSlimFolderId(selection.path)
}

function slimFolderIdFromSelection(selection: SidebarSelection & { kind: 'folder' }): SlimFolderId {
  return selection.path === '' ? 'memory' : (selection.path as SlimFolderId)
}

function isSlimEntryInFolder(entry: VaultEntry, slimFolder: SlimFolderId, rootPath: string | undefined): boolean {
  const relativePath = pathRelativeToRoot(entry.path, rootPath)
  if (relativePath === null) return false
  if (slimFolder === 'memory') return relativePath === 'MEMORY.md'
  return isInFolder(relativePath, slimFolder)
}
