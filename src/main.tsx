...
// DREAMFORGE_SLIM: Sentry import 物理删除 (PR 8)
...
// DREAMFORGE_SLIM: TLDRAW_CONTEXT_MENU_SELECTOR 物理删除 (PR 8, tldraw dep 删)
...
// DREAMFORGE_SLIM: isTldrawContextMenuTarget + preventNativeContextMenu 物理删除 (PR 8, tldraw dep 删)
...
if ('__TAURI__' in window || '__TAURI_INTERNALS__' in window) {
  // DREAMFORGE_SLIM: tldraw exemption 删 (PR 8), inline preventDefault
...
window.__laputaTest = {
  ...
  async triggerMenuCommand(id: string) {
    ...
    if (!window.__laputaTest?.dispatchBrowserMenuCommand) {
      throw new Error('DreamX test bridge is missing dispatchBrowserMenuCommand')
    }
...
// DREAMFORGE_SLIM: sentryReactErrorHandler 物理删除 (PR 8, telemetry 全 no-op)
...
function captureReactRootError(
  ...
  // DREAMFORGE_SLIM: sentryReactErrorHandler 调用物理删除 (PR 8)
