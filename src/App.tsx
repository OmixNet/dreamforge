...
// DREAMFORGE_SLIM: useDeepLinks 物理删除 (PR 4)，保留 noop prop 兼容
...
// DREAMFORGE_SLIM: SETTINGS_SECTION_IDS 物理删除 (PR 6, handleOpenVaultSettings 已删)
import {
  vaultPathForEntry,
...
// DREAMFORGE_SLIM: useMcpSetupDialogController 物理删除 (PR 6)
import { DreamPanel } from './components/DreamPanel'
import { SlimSidebar } from './components/SlimSidebar'
import { type SlimFolderId } from './utils/slimSelection'
import { DREAMFORGE_SLIM_MODE } from './lib/dreamforgeMode'
import { sidebarSelectionForSlimFolder } from './utils/slimSelection'
import {
...
const DEFAULT_SELECTION: SidebarSelection = DREAMFORGE_SLIM_MODE ? { kind: 'folder', path: 'notes' } : INBOX_SELECTION
...
function MainApp({ noteWindowParams }: { noteWindowParams: NoteWindowParams | null }) {
  ...
  // DREAMFORGE_SLIM: active folder in the slim 5-entry sidebar (Notes / Wiki / Memory / Raw / Archive)
  const [slimFolderSelection, setSlimFolderSelection] = useState<SlimFolderId | null>(
    DREAMFORGE_SLIM_MODE ? 'notes' : null,
  )
  ...
  const aiAgentsStatus = useAiAgentsStatus({
    enabled: aiFeaturesEnabled && !DREAMFORGE_SLIM_MODE && !noteWindowParams && !aiWorkspaceWindow,
  })
  const aiAgentsOnboarding = useAiAgentsOnboarding(
    aiFeaturesEnabled && !DREAMFORGE_SLIM_MODE && onboarding.state.status === 'ready' && !noteWindowParams && !aiWorkspaceWindow,
  )
  ...
  const handleSlimFolderSelect = useCallback((folder: SlimFolderId) => {
    setSlimFolderSelection(folder)
    handleSetSelection(sidebarSelectionForSlimFolder(resolvedPath, folder))
  ...
  const {
    ...
    resolvedPath,
    windowMode: Boolean(noteWindowParams) || aiWorkspaceWindow,
    // v0.2 PR 11.5: slim mode auto-inits git silently — no "Enable Git?" dialog
    slimMode: true,
  })
  ...
  // DREAMFORGE_SLIM: mcpSetupDialog = useMcpSetupDialogController(...) 物理删除 (PR 6)
  ...
  // DREAMFORGE_SLIM: open MEMORY.md (or create a fresh one if the engine hasn't
  // written it yet) and switch the note list to show all entries (so wiki/ files
  // are visible). Both land in the editor through the same path as a sidebar
  // selection.
  // Fix eslint react-hooks/preserve-manual-memoization: notes object 每次 render 变
  // (useNoteActions 不 useMemo 包), useCallback dep 引用 unstable → ref + effect 同步 handleCreateNote
  const handleCreateNoteRef = useRef(notes.handleCreateNote)
  useEffect(() => {
    handleCreateNoteRef.current = notes.handleCreateNote
  })
  const handleOpenMemory = useCallback(() => {
    setSlimFolderSelection('memory')
    handleSetSelection(sidebarSelectionForSlimFolder(resolvedPath, 'memory'))
    const memoryPath = `${resolvedPath}/MEMORY.md`
    const existing = visibleEntries.find((entry) => entry.path === memoryPath)
    if (existing) {
      void handleReplaceActiveTab(existing)
      return
    }
    handleCreateNoteRef.current('MEMORY', 'Memory')
  }, [resolvedPath, visibleEntries, handleSetSelection, handleReplaceActiveTab])
  ...
  // DREAMFORGE_SLIM: pullAndPush = autoSync.pullAndPush 物理删除 (PR 6)
  ...
  // DREAMFORGE_SLIM: handlePullAndPushSelectedRepository 物理删除 (PR 6, StatusBar 不再传 onPullAndPush)

  // DREAMFORGE_SLIM: pullAndPush = autoSync.pullAndPush 物理删除 (PR 6, 唯一 user handlePullAndPushSelectedRepository 已删)
  ...
  const {
    // DREAMFORGE_SLIM: buildNumber 物理删除 (PR 6, StatusBar 不再传 onCheckForUpdates)
    diffToggleRef,
    findInNoteRef,
  ...
  const {
    ...
    selectedVaultPath,
    settingsLoaded,
    // DREAMFORGE_SLIM: showMcpSetupDialog 物理删除 (PR 6)
  ...
  const commands = useAppCommands({
    ...
    // DREAMFORGE_SLIM: mcpStatus + onInstallMcp 物理删除 (PR 6)
  ...
  const aiWorkspaceSurface = (
    <AppAiWorkspaceSurface
      ...
      // defaultAiAgent + defaultAiTarget + defaultAiAgentReadiness + defaultAiAgentReady
  ...
  if (shouldShowStartupScreen) {
    return (
      <StartupScreen
        ...
        // DREAMFORGE_SLIM: showMcpSetupDialog prop 物理删除 (PR 6)
  ...
  if (aiWorkspaceWindow) {
    return (
      <AppPreferencesProvider dateDisplayFormat={dateDisplayFormat}>
        {!DREAMFORGE_SLIM_MODE && aiWorkspaceSurface}
  ...
  return (
    <AppPreferencesProvider dateDisplayFormat={dateDisplayFormat}>
      <div className="app-shell">
        <div className="app">
          {sidebarVisible && (
            DREAMFORGE_SLIM_MODE ? (
              <SlimSidebar
                vaultPath={resolvedPath}
                entries={visibleEntries}
                selection={slimFolderSelection}
                onSelect={handleSlimFolderSelect}
                locale={appLocale}
              />
            ) : (
              <>
          ...
          <div className={`app__editor${aiActivity.highlightElement === 'editor' || aiActivity.highlightElement === 'tab' ? ' ai-highlight' : ''}`}>
            <Editor
              ...
              onCopyDeepLink={activeDeletedFile ? undefined : () => {}}
        ...
        {/* DREAMFORGE_SLIM: PR 6 StatusBar cleanup — 只传 vault 路径/设置/主题/本地 commit 4 项 + 状态指示 */}
        <StatusBar
          vaultPath={resolvedPath}
          ...
          modifiedCount={gitModifiedCount}
          onCommitPush={handleCommitPush}
          commitActionPending={commitFlow.isOpeningCommitDialog}
          isOffline={networkStatus.isOffline}
          isVaultReloading={vault.isReloading || isVaultContentLoading}
          themeMode={documentThemeMode}
          onToggleThemeMode={settingsLoaded ? handleToggleThemeMode : undefined}
          locale={appLocale}
        />
        {aiFeaturesEnabled && !effectiveShowAIChat && !DREAMFORGE_SLIM_MODE ? (
