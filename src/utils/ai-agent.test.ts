...
describe('buildAgentSystemPrompt', () => {
  it('returns preamble when no vault context', () => {
    ...
    expect(prompt).toContain('working inside DreamX')
  ...
  it('points safe-mode agents to bundled DreamX docs without shell commands', () => {
    ...
    expect(prompt).toContain("Portent as DreamX's default best-practice model")
  ...
  it('does not promise shell execution for Pi power user mode', () => {
    ...
    expect(prompt).toContain('Pi currently uses the same conservative DreamX MCP configuration')
