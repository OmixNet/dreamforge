...
describe('buildAgentSystemPrompt', () => {
  it('returns preamble when no vault context', () => {
    ...
    expect(prompt).toContain('working inside DreamForge')
  ...
  it('points safe-mode agents to bundled DreamForge docs without shell commands', () => {
    ...
    expect(prompt).toContain("Portent as DreamForge's default best-practice model")
  ...
  it('does not promise shell execution for Pi power user mode', () => {
    ...
    expect(prompt).toContain('Pi currently uses the same conservative DreamForge MCP configuration')
