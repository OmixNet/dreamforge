...
const DEFAULT_MOCK_VAULT_PATH = '/Users/biomatrix/Desktop/APP/dreamforge-test-vault'
const DEFAULT_MOCK_VAULT = {
  label: 'dreamforge-test-vault',
  path: DEFAULT_MOCK_VAULT_PATH,
}

// DREAMFORGE_SLIM: default mock vault is dreamforge-test-vault (5-entry structure)
...
export const mockHandlers: Record<string, (args: any) => any> = {
  ...
  check_vault_exists: (args: { path: string }) => {
    // In mock mode, keep the DreamForge test vault and the legacy demo vault available.
    return args.path.includes('dreamforge-test-vault') || args.path.includes('demo-vault-v2')
  },
